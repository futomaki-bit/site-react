const cover = (<div class="jumbotron cover tealtheme parent">
    <div class="container pt-5">
        <div class="row">
            <div class="column col-sm-12 child">
                <a href="https://github.com/futomaki-bit">
                    <h2><i class="fab fa-github"></i><span> </span>
                        GitHub
                    </h2>
                </a>
            </div>
            <div class="column col-sm-12 tallchild">
                <div class="container text-center pt-5">
                    <a href="https://github.com/futomaki-bit">
                        <img src="https://avatars.githubusercontent.com/u/72528988?v=4" class="profilepic" alt="futomakiyoin"
                        />
                    </a>
                    <div>
                        <a href="https://github.com/futomaki-bit">
                            <h2 class="mt-4">futomaki-bit</h2>
                        </a>
                        <p class="small">Hello, World! Hello, You!</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>);

export default cover;